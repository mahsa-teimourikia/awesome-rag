  ...
  elements.learningPathList.innerHTML = learningPath.map((track) => {
    ...
    <section class="learning-track ${track.tone}" id="${track.id}">
      ...
      <ol class="learning-modules">${modules.map((module, index) => `
        <li class="learning-module" id="lesson-${module.id}"><span class="module-number">${index + 1}</span><div><h4><a class="lesson-title" href="#lesson-${module.id}">${module.title}</a></h4><p>${module.description}</p><div class="module-meta"><span>${module.minutes} min</span>${module.technologies.map((technology) => `<span>${technology}</span>`).join("")}${progress.quizScores?.[module.id] !== undefined ? `<span class="score-badge">Score ${progress.quizScores[module.id]}%</span>` : ""}</div><div class="module-links"><a href="${resourceHref(module.material)}" target="_blank" rel="noreferrer">Read lesson ↗</a><a href="${resourceHref(module.notebook)}" target="_blank" rel="noreferrer">Open notebook ↗</a>${module.implementation ? `<a href="${resourceHref(module.implementation)}" target="_blank" rel="noreferrer">Open implementation ↗</a>` : ""}<a href="#quiz-${module.id}">Quiz checkpoint</a><button class="complete-button" data-lesson-id="${module.id}" type="button">${isLessonComplete(module.id) ? "Completed" : "Mark complete"}</button></div></div></li>
      `).join("")}</ol>
    </section>
  `;
  }).join("");
...
function showLessonFromHash() {
  ...
  elements.lessonDetail.innerHTML = `<div class="lesson-detail-top"><span class="level-pill">${lesson.level}</span><a class="text-button" href="#learning-path">Back to path</a></div><h2 id="lesson-detail-heading">${lesson.title}</h2><p class="lesson-outcome">${lesson.description}</p><div class="lesson-facts"><span>${lesson.minutes} minutes</span>${lesson.technologies.map((item) => `<span>${item}</span>`).join("")}${score !== undefined ? `<span class="score-badge">Checkpoint score ${score}%</span>` : ""}</div>${contentMarkup}<div class="lesson-actions"><a class="primary-button" href="${resourceHref(lesson.material)}" target="_blank" rel="noreferrer">Open full lesson ↗</a><a class="secondary-button" href="${resourceHref(lesson.notebook)}" target="_blank" rel="noreferrer">Run guided notebook ↗</a>${lesson.implementation ? `<a class="secondary-button" href="${resourceHref(lesson.implementation)}" target="_blank" rel="noreferrer">Open implementation ↗</a>` : ""}<a class="secondary-button" href="#quiz-${lesson.id}">Take checkpoint quiz</a></div><p class="lesson-tip">The core concepts are included above. Use the linked lesson, notebook, and implementation for the complete experiments and exercises.</p>${next ? `<a class="next-lesson" href="#lesson-${next.id}">Next: ${next.title} →</a>` : `<p class="next-lesson">You reached the end of the learning path.</p>`}`;
      ...
      return `
        <article class="question-card" data-question-id="${question.id}" ${categoryAnchor}>
          ...
          <div class="review-panel" hidden>
            ...
            <p>
              <a href="${repositoryContentBase}${question.source.url}" target="_blank" rel="noreferrer">
