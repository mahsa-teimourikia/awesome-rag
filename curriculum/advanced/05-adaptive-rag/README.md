...
**Stage:** Advanced Systems & Agents
**Focus:** Pre-Retrieval Routing

Standard RAG architectures process every query through the exact same pipeline. Whether a user asks "Hello" or "How does the Q3 deployment affect the European vendor compliance?", standard RAG will generate vector embeddings and search the database. This is a waste of compute, adds unnecessary latency to simple queries, and often results in poor answers when the query requires a different strategy (like web search).

**Adaptive RAG** solves this by inserting an LLM Router *before* retrieval. The router evaluates the complexity and intent of the query, and conditionally routes it to the most appropriate sub-system.

## Learning Objectives

After this module, you will be able to:
1. Understand the difference between Pre-Retrieval (Adaptive RAG) and Post-Retrieval (Corrective RAG) routing.
2. Build an LLM Router to classify query intent (e.g., direct answer, web search, internal search).
3. Implement an Adaptive RAG state machine using **LangGraph**.
4. Optimize RAG pipelines for cost and latency by bypassing heavy retrieval for simple queries.

## Module Contents

*   `06_adaptive_rag.ipynb`: A deep dive into query classification and building a LangGraph router.
