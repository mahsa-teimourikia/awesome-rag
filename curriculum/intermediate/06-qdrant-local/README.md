...
run the same retrieval evaluation
    ↓
atomically promote support_docs_current alias
    ↓
...
The lab creates `support_docs_v2` with a different vector dimension and embedding configuration, re-ingests the source records with updated metadata, validates schema and point count, reruns evaluation, and promotes an alias. V1 remains available for rollback.

The measured V1/V2 metrics are generated by executing the notebook. They are not hand-authored benchmark claims. A production release gate should use a representative corpus and query distribution, slice results, latency percentiles, capacity data, and failure tests.
...
5. Build `support_docs_v3` with a changed chunking policy and define a promotion gate.
