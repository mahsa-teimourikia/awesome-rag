...
The notebook packages those conditions in a side-effect-free helper:

```python
assessment = evaluate_collection_migration(...)
```

It returns `decision`, `blockers`, `warnings`, `baseline_metrics`, and `candidate_metrics`. This follows the explicit release-report pattern taught in [Intermediate 04 — Evaluation](../04-evaluation/README.md): evidence assessment is reusable and inspectable, while alias mutation remains a separate application action. A `BLOCK` decision never reaches the promotion branch; `REVIEW_WARNINGS` requires review rather than automatic promotion.
